package com.books.database.service;

import com.books.database.interfaces.Service;
import com.books.database.repository.Dao;
import com.books.database.repository.ReportDao;
...
public abstract class AbstractService<T, D extends Dao<T>> extends Parameterized<T> implements Service<T> {
    ...
    @Override
    public List<T> getAllEntities(ReportFilter reportFilter, Integer firstResult, Integer maxResult, String[] include) {
        if (reportFilter.getOptions().isExport()) {
            reportFilter.getOptions().setPageNumber(null);
            reportFilter.getOptions().setPageSize(null);
            return getAllEntitiesWithoutMaxLimit(reportFilter, include);
        } else {
            return reportDao.reportEntityWithIncludeJoin(reportFilter, firstResult, maxResult, include, getClazz());
        }
    }
