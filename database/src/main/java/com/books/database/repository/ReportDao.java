package com.books.database.repository;

import com.books.database.statics.constants.Parameterized;
import com.books.utility.commons.repository.dto.*;
...
import com.books.utility.commons.statics.constants.UtilityConstant;
import com.books.utility.support.ValidationEngine;
...
public class ReportDao<T> extends Parameterized<T> {
    ...
    public Integer countEntity(ReportFilter reportFilter) {
        return this.countEntity(reportFilter, getClazz());
    }

    public Integer countEntity(ReportFilter reportFilter, Class<T> clazz) {
    ...
    public Integer countEntityGroupBy(ReportFilter reportFilter, List<String> groupBy, List<String> selects, Class<T> clazz) {
        CriteriaBuilder criteriaBuilder = getEntityManager().getCriteriaBuilder();
    ...
    public List<T> reportEntity(ReportFilter reportFilter) {
        return this.reportEntity(reportFilter, getClazz());
    }

    public List<T> reportEntity(ReportFilter reportFilter, Class<T> clazz) {
        CriteriaBuilder criteriaBuilder = getEntityManager().getCriteriaBuilder();
        ...
        int maxResult = UtilityConstant.MAXIMUM_REPORT_PAGE_SIZE < reportFilter.getOptions().getPageSize() ? UtilityConstant.MAXIMUM_REPORT_PAGE_SIZE : reportFilter.getOptions().getPageSize();
    ...
    public <R> List<R> reportEntityGroupBy(ReportFilter reportFilter, List<String> groupBy, Class<T> root, Class<R> output) {
    ...
    public <R> List<R> reportEntityGroupBy(ReportFilter reportFilter, List<String> groupBy, List<String> selects, Class<T> root, Class<R> output) {
        CriteriaBuilder criteriaBuilder = getEntityManager().getCriteriaBuilder();
    ...
    public List<T> reportEntityWithInclude(ReportFilter reportFilter, String[] include) {
        return this.reportEntityWithInclude(reportFilter, include, getClazz());
    }

    public List<T> reportEntityWithInclude(ReportFilter reportFilter, String[] include, Class<T> clazz) {
        CriteriaBuilder criteriaBuilder = getEntityManager().getCriteriaBuilder();
        CriteriaQuery<T> criteriaQuery = criteriaBuilder.createQuery(clazz);
        Root<T> rootQuery = criteriaQuery.from(clazz);
    ...
    public List<T> reportEntityWithIncludeJoin(ReportFilter reportFilter, String[] include) {
        return this.reportEntityWithIncludeJoin(reportFilter, include, getClazz());
    }


    public List<T> reportEntityWithIncludeJoin(ReportFilter reportFilter, String[] include, Class<T> clazz) {
        CriteriaBuilder criteriaBuilder = getEntityManager().getCriteriaBuilder();
        CriteriaQuery<T> criteriaQuery = criteriaBuilder.createQuery(clazz);
        Root<T> rootQuery = criteriaQuery.from(clazz);
        criteriaQuery.select(rootQuery);
        ...
        Dao.addAttributesToEntityGraph(eg, include, columns);
        createSortExpression(criteriaQuery, rootQuery, criteriaBuilder, reportFilter.getOptions());
        TypedQuery<T> query = getEntityManager().createQuery(criteriaQuery.distinct(reportFilter.getOptions().isDistinct()));
        ...
        if (reportFilter.getOptions().getPageSize() != null && reportFilter.getOptions().getPageSize() > 0) {
            ...
            query.setMaxResults(UtilityConstant.DEFAULT_REPORT_PAGE_SIZE);
        }
        if (reportFilter.getOptions().getPageNumber() == null || reportFilter.getOptions().getPageNumber() < 0) {
            query.setFirstResult(UtilityConstant.DEFAULT_REPORT_PAGE_NUMBER);
        }
    ...
    public List<T> reportEntityWithIncludeJoin(ReportFilter reportFilter, Integer firstResult, Integer maxResult, String[] include, Class<T> clazz) {
        CriteriaBuilder criteriaBuilder = getEntityManager().getCriteriaBuilder();
        CriteriaQuery<T> criteriaQuery = criteriaBuilder.createQuery(clazz);
        Root<T> rootQuery = criteriaQuery.from(clazz);
        criteriaQuery.select(rootQuery);
        ...
        if (maxResult != null && maxResult > 0) {
            query.setMaxResults(UtilityConstant.MAXIMUM_REPORT_PAGE_SIZE < maxResult ? UtilityConstant.MAXIMUM_REPORT_PAGE_SIZE : maxResult);
        } else {
            query.setMaxResults(UtilityConstant.DEFAULT_REPORT_PAGE_SIZE);
        }
    ...
    public List<T> reportEntityWithIncludeJoinWithSizeCheck(ReportFilter reportFilter, String[] include, Class<T> clazz) {
        CriteriaBuilder criteriaBuilder = getEntityManager().getCriteriaBuilder();
    ...
    public List<T> reportEntityWithIncludeJoinWithoutMaxLimit(ReportFilter reportFilter, String[] include, Class<T> clazz) {
        CriteriaBuilder criteriaBuilder = getEntityManager().getCriteriaBuilder();
        CriteriaQuery<T> criteriaQuery = criteriaBuilder.createQuery(clazz);
        Root<T> rootQuery = criteriaQuery.from(clazz);
        criteriaQuery.select(rootQuery);
    ...
    public <R> List<R> reportHqlWithIncludeJoin(Class<R> cls, String hql, ReportOption reportOption, Map<String, Object> parameters, String[] include) {
        ...
        Dao.addAttributesToEntityGraph(eg, include, columns);
        int maxResult = UtilityConstant.MAXIMUM_REPORT_PAGE_SIZE < reportOption.getPageSize() ? UtilityConstant.MAXIMUM_REPORT_PAGE_SIZE : reportOption.getPageSize();
