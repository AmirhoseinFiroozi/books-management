...
import com.books.utility.commons.repository.dto.ConditionParameters;
...
import static com.books.utility.commons.statics.constants.UtilityConstant.MAXIMUM_REPORT_PAGE_SIZE;
...
public class Dao<T> extends Parameterized<T> {
    ...
    //uses hibernate mapping for entity class configured in the query itself
    public <T> List<T> querySql(org.hibernate.query.Query query, Map<String, Object> parameters) {
        for (Map.Entry<String, Object> pair : parameters.entrySet())
            query.setParameter(pair.getKey(), pair.getValue());
        return query.getResultList();
    }
    ...
    // this one uses hibernate entity mapping
    public <T> List<T> reportEntityWithSqlQuery(String select, String groupBy, ReportFilter reportFilter) {
        String where = generateWhereClause(reportFilter.getConditions(), true);
        String sql = select;
        if (!where.equals(" where ")) {
            if (select.contains("where")) {
        ...
        sql += generateSqlReportOption(reportFilter.getOptions());
        NativeQuery query = this.getSession().createNativeQuery(sql, getClazz());
